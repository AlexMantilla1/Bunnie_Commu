...
uint8_t deviceSlaveAdd = 0;
uint16_t wait_us = 1000/3; 		// default value for 1 ms period.

uint16_t sdaPIN = 1;		// Pin for sda, default: 1
uint16_t sclPIN = 2;		// Pin for scl, default: 2

uint8_t slaveToSearch = 1;
...
uint16_t i2cSlaveListeningCopying(uint32_t timeout, uint8_t* dataStored, uint16_t* sdaPines) {	//try to use a small timeout (how much?....)
	...
	********************************/
	...
	uint8_t slaveAddReaded = 0, dataAdd = 0;
	uint16_t dataReaded = 0;
	bool row = 1; // read or write flag, default read
	uint16_t slaveConnected;
	uint16_t syncAns = 0;
...
/*
void printMemory(uint8_t* dataStored) {
...
uint16_t i2cMasterCheck( uint8_t* dataStored, uint16_t* sdaPines , uint8_t* sdaToFind) {
	...
	*  2. Once the device was found (responds the transaction), reads the address SLAVE_MEMORY_SDA_MASTER.
	...
	********************************/
	...
	uint8_t slaveAddressToSearch = dataStored[MASTER_MEMORY_S1_ADD + slaveToSearch - 1];
	bool slaveConnected = slaveAddressToSearch & 0x1;
	uint8_t readValue = 0;
	bool slaveFound = false;
	uint16_t message = DEVICE_NOT_FOUND;
					...
					if( dataStored[i] == 0 ) {
						...
						do {
							for (char l = 0 ; l < NUM_OF_TRIES2 ; l++) {
								uint16_t algo = i2cMasterRead(slaveAddress, SLAVE_MEMORY_SDA_SLAVE, &readValue);
